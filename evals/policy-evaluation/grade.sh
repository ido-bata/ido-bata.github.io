...
#   - negative    : projects_first_marker present AND canonical_surface absent
#   - positive    : canonical_surface present  AND projects_first absent
#   - unknown     : どれも成立しない (grader が invariant を判別できない)
#
# 使い方:
...
case "$classified" in
  ...
  *)
    ...
    echo "  canonical_surface=$canonical_surface  projects_first=$projects_first  regression=$regression_marker" >&2
    exit 1
    ;;
esac
