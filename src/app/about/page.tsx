        ...
        <div className={cx(grid({ cols: 12, gap: 6 }))}>
          <div className={cx(stack({ gap: 5 }), css({ gridColumn: { base: "1", md: "span 7" } }))}>
            ...
            <div className={cx(stack({ gap: 4 }), css({ maxW: "52ch" }))}>
              ...
              >
                いど端は Discord で動いてる小さいサーバー。 交流より、 映像 / プログラミング /
                デザインみたいな関心領域のことを調べて書く場所として使ってる。
              </p>
              <p
                className={css({
                  fontSize: { base: "md", md: "lg" },
              ...
              >
                ウェルカムも入会の儀式も作ってない。 誰が来たかは誰にも分からない、
                投稿を誰が見てるかを気にする必要もないようにしてる。 ひとりごと / wip は
                反応より書くことを優先するチャネルで、 書きたいことをそのまま書けるようにしてる。
              </p>
              <p
                className={css({
                  fontSize: { base: "md", md: "lg" },
              ...
              >
                名前は井戸端会議の響きだけ借りてる。 うちとしては交流の場じゃなくて、
                情報共有のためのサーバーってとこでやってる。
              </p>
            </div>
          </div>
