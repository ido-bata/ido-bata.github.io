...
export const rules: RulesContent = {
  ...
  philosophy: {
    ...
    title: "サーバの理念",
    body:
      "ido-bata Discord サーバは、Web サイトや関連リポジトリと同じく「実利重視・情報共有主体・自由参加脱退」を理念とする。議論は「誰の何の課題をどう解決するか」を軸に進め、感想や世間話だけで終わるチャネルは基本的に用意しない。各自の発言・知見・成果物はコミュニティの資産として公開・共有する前提で扱う。",
  },
  ...
  prohibited: {
    ...
    title: "禁止行為",
    body:
      "以下は歓迎しない。悪意の有無に関わらず、起こった時点で運営に通知し、改善が見られない場合は参加の制限を検討する。",
    bullets: [
  ...
  enforcement: {
    ...
    title: "違反時の対応",
    body:
      "違反を見つけた場合、または違反の対象になった場合は、チャンネルに晒さず運営に通報する。通報窓口と運用フローは行動規範 (`docs/code-of-conduct.md`) §6 に従う。",
    bullets: [
  ...
  meta: {
    ...
    title: "改定・問い合わせ",
    body:
      "本ルールはコミュニティの運用状況に応じて不定期に見直す。改定は本リポジトリの `docs/code-of-conduct.md` / Web ページの更新 Pull Request で行い、Discord サーバ内と本ページ上部に告知する。問い合わせ窓口は行動規範 §6 に集約する。",
  },
};